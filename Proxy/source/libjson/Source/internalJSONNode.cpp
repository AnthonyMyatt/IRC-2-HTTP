...
internalJSONNode::internalJSONNode(const internalJSONNode & orig) json_nothrow :
    _type(orig._type), _name_encoded(orig._name_encoded), _name(orig._name),
...
internalJSONNode::internalJSONNode(const json_string & unparsed) json_nothrow : _type(), _name_encoded(false), _name(), _string(unparsed), _string_encoded(), _value()
    initializeMutex(0)
    initializeRefCount(1)
